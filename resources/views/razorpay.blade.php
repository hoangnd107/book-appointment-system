                ...
                <div id="razorpay_pay" class="payfrom">
                    ...
                    <form action="{!! route('razor-payment') !!}" method="GET">
                        @if (Request::get('type') != 3)
                            ...
                            @else
                            <input id="amount" name="amount" type="hidden" value="{{ Request::get('amount') }}">
                        @endif
                        <input id="session_id" name="session_id" type="hidden" value="{{ Session::get('type') }}">
                        <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="{{ $paymentdetail['razorpay_razorpay_key'] }}"
                            data-amount="{{ (int) $amount * 100 }}" data-buttontext='{{ __('message.Pay') }}' data-name="{{ env('APP_NAME') }}"
                            data-description="Payment" data-image="{{ asset('public/image_web/896814.png') }}" data-prefill.name="name"
